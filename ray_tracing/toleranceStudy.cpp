...
int main(int argc, char **argv)
{
	...
	TH1D *h1_zmiss[2];
	h1_zmiss[0] = new TH1D("z-miss, direct","h1_zmiss_dir",1000,-5,5);
	h1_zmiss[1] = new TH1D("z-miss, refl/refr","h1_zmiss_ref",1000,-5,5);
	...
	*/
	...
	while(tries<=num_tries){
		...
		double thisZ = random->Uniform(-range, 0.)+offset_depth;
		...
		if(outputs.size()>0){
			...
			h2_vertices->Fill(abs(diff_2d.GetX()/1.e3), (source_2d.GetZ()-offset_depth)/1.e3); // [m]
